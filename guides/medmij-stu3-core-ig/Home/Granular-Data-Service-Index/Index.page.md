...
- **Overview**
    ...
    - **System role(s)** - the system roles corresponding to the different transactions within each data service. Each system is of the form 'MMC-[CIM abbreviation]\[Transaction indicator\](-[Suffix])-[Data service version (abbreviated)]', where:
        ...
        - the Data service version is abbreviated for prereleases (i.e. if the data service is released in an alpha, beta or release candidate version). For these, only the latter part of the data service version is included. For instance, if the data service version is '1.0.0-alpha.1', the abbreviated version is 'alpha.1'.
